...
pasar_a_terminos_reales <- function(df, col_fecha, col_valor, tabla_ipc) {
  ...
  message(sprintf("✅ Columna generada: %s (Base: %s)", var_treales, format(mes_base, "%Y-%m")))
  if (registros_sin_ipc > 0) {
    message(sprintf("⚠️ Advertencia: %d registros tienen fecha válida pero no encontraron dato en tabla_ipc.", registros_sin_ipc))
  }
